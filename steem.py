...
def main(argv):
    ...
    while True:
        ...
        try:
           ...
           print("There is contectivity :) \n")
        except ConnectionError:
            print("No conectivity you broke as Nigga!\n")
            continue
        ...
        btc_xcn = conn.api_query("returnOrderBook",{'currencyPair':'BTC_STEEM'})
        # parity three whole object
        eth_xcn = conn.api_query("returnOrderBook",{'currencyPair':'ETH_STEEM'})
        # last ask parity one
        ...
        if(snt > eth_btc_AF):
            def tran (qt):
                ...
                qtE=qtE/eth_xcn_A
                orderNumber2 = conn.buyI("ETH_STEEM",eth_xcn_A,qtE,1)
                print("Order 2: %s placed , Piece of shit!! "%orderNumber2)
                qtX=float(balances["STEEM"])
                orderNumber3 = conn.sellI("BTC_STEEM",btc_xcn_B,qtX,1)
        ...
        print "Time stamp {:%Y-%m-%d %H:%M:%S}".format(datetime.datetime.now())+" Period: %ss\n"%(period)
