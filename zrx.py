...
def main(argv):
    ...
    while True:
        ...
        btc_xcn = conn.api_query("returnOrderBook",{'currencyPair':'BTC_ZRX'})
        # parity three whole object
        eth_xcn = conn.api_query("returnOrderBook",{'currencyPair':'ETH_ZRX'})
        # last ask parity one
        ...
        if(snt > eth_btc_AF):
            def tran (qt):
                ...
                qtE=qtE/eth_xcn_A
                orderNumber2 = conn.buyI("ETH_ZRX",eth_xcn_A,qtE,1)
                print("Order 2: %s placed , Piece of shit!! "%orderNumber2)
                balances = conn.returnBalances()
                qtX=float(balances["ZRX"])
                qtBTC= float
                orderNumber3 = conn.sellI("BTC_ZRX",btc_xcn_B,qtX,1)
